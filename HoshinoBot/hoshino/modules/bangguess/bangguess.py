...
sv = Service('bangguess', visible=True)

PIC_SIDE_LENGTH = 35 
ONE_TURN_TIME = 20
DB_PATH = os.path.expanduser('~/.hoshino/pcr_bangdream_guess_winning_counter.db')
BLACKLIST_ID = [1072, 1908, 4031, 9000]
bangdreamlist = list(pcr_duel.bangdreamlist)
...
async def bang_guess(bot, ev: CQEvent):
    try:
        ...
        img = c.icon.open()
        left = math.floor(random.random()*(181-PIC_SIDE_LENGTH))
        upper = math.floor(random.random()*(181-PIC_SIDE_LENGTH))
        ...
        msg = f'猜猜这个图片是哪位角色的一部分?({ONE_TURN_TIME}s后公布答案){image}'
