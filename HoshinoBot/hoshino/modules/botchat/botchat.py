...
async def chat_sad(bot, ev):
    prob = random.random()
    if prob < 0.33:
        await bot.send(ev, '哼！你才是笨蛋呢\n'+str(R.img(f"keyword/yls.gif").cqcode), at_sender=True)
    elif 0.33 < prob < 0.66:
        await bot.send(ev, '哼！你才是笨蛋呢\n'+str(R.img(f"keyword/serena.gif").cqcode), at_sender=True)
    else:
        await bot.send(ev, '哼！你才是笨蛋呢\n'+str(R.img(f"keyword/hiiro.png").cqcode), at_sender=True)

@sv.on_keyword(('笨蛋'), only_to_me=True)
async def chat_sadsad(bot, ev):
    prob = random.random()
    if prob < 0.33:
        await bot.send(ev, '哼！你才是笨蛋呢\n'+str(R.img(f"keyword/yls.gif").cqcode), at_sender=True)
    elif 0.33 < prob < 0.66:
...
@sv.on_keyword(('沙雕机器人', '傻逼机器人'))
async def chat_sb(bot, ev):
    if random.random() < 0.50:
        await util.silence(ev, 60, skip_su=False)
        await bot.send(ev, R.img(f"keyword/kaca.gif").cqcode) 
    else:
        await util.silence(ev, 60, skip_su=False)
        await bot.send(ev, R.img(f"keyword/锤子.gif").cqcode)     
        
@sv.on_keyword(('傻逼', '沙雕'), only_to_me=True)
async def chat_sbsb(bot, ev):
    if random.random() < 0.50:
        await util.silence(ev, 60, skip_su=False)
        await bot.send(ev, R.img(f"keyword/kaca.gif").cqcode) 
    else:
...
@sv.on_keyword(('憨憨机器人', '憨批机器人'))
async def chat_han(bot, ev):
    prob = random.random()
    if prob < 0.33:
        await bot.send(ev, '哼！你才憨\n'+str(R.img(f"keyword/yls.gif").cqcode), at_sender=True)
    elif 0.33 < prob < 0.66:
        await bot.send(ev, '哼！你才憨\n'+str(R.img(f"keyword/serena.gif").cqcode), at_sender=True)
    else:
        await bot.send(ev, '哼！你才憨\n'+str(R.img(f"keyword/hiiro.png").cqcode), at_sender=True)       

@sv.on_keyword(('憨憨', '憨批'), only_to_me=True)
async def chat_hanhan(bot, ev):
    prob = random.random()
    if prob < 0.33:
        await bot.send(ev, '哼！你才憨\n'+str(R.img(f"keyword/yls.gif").cqcode), at_sender=True)
    elif 0.33 < prob < 0.66:
...
@sv.on_keyword(('爬', '爪巴'), only_to_me=True)
async def chat_pa(bot, ev):
    prob = random.random()
    if prob < 0.30:
        await bot.send(ev, R.img(f"keyword/爬1.jpg").cqcode)  
    elif 0.30 < prob < 0.60:
...
@sv.on_keyword(('lol', '来劲'))
async def lol(bot, ev):
    if random.random() < 0.10:
        await bot.send(ev, R.img(f"keyword/lol.jpg").cqcode)      
...
async def anti_qks(bot, ev):
    await util.silence(ev, 60, skip_su=False)
    await bot.delete_msg(self_id=ev.self_id, message_id=ev.message_id)
    await bot.send(ev, f'骑空士爪巴', at_sender=True)

#=====反猴楼=====#
@sv.on_keyword(('holo','木口'))
async def anti_holo(bot, ev):
    await util.silence(ev, 60, skip_su=False)
    await bot.delete_msg(self_id=ev.self_id, message_id=ev.message_id)
...
async def anti_vtb(bot, ev):
    if random.random() < 0.90:
        await bot.send(ev, R.img(f"keyword/ybb.jpg").cqcode)  
    else:
        await bot.send(ev, R.img(f"keyword/ybbb.jpg").cqcode)        
