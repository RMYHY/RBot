...
GACHA_COST_Fail = 300 #抽老婆失败补偿量
ZERO_GET_AMOUNT = 300  # 没钱补给量
...
JiaoYi_LIMIT = 5 #一天进行的女友交易次数上限
#转账部分
Zhuan_Need = 0.5 #转账所需的手续费比例
Zhuan_DAILY_LIMIT = 100000 #每日转账金币上限（扣除手续费前）
Zhuan_Low_LIMIT = 10000 #最低转账额度（扣除手续费前）
#声望兑换部分
SW_TO_GOLD = 100 #1声望兑换的金币数
SW_DAILY_LIMIT = 1000 #每日使用声望兑换的最大额度

#胜负声望部分
WinSWBasics = 500 #赢了获得的基础声望
LoseSWBasics = 200 #输了掉的基础声望
#签到部分
scoreLV = 500 #每日根据等级获得的金币（等级*参数）
SWLV = 100 #每日根据等级获得的声望（等级*参数）
